...
(async () => {
  const configPromise =
    typeof config !== "undefined"
      ? config
      : window.spotConfigPromise || fetch("/config/config.json").then((res) => res.json());
  const matchConfigPromise =
    typeof matchScoutingConfig !== "undefined"
      ? matchScoutingConfig
      : window.spotMatchScoutingConfigPromise ||
        fetch("/config/match-scouting.json").then((res) => res.json());

  config = await configPromise;
  matchScoutingConfig = await matchConfigPromise;
  ...
  function updateLastAction() {
    ...
    const lastActionsEl =
      document.getElementById("last-actions") ||
      document.querySelector(".status .last-actions");
    if (!lastActionsEl) return;

    lastActionsEl.innerText = lastActions
      .reverse()
